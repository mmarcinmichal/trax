...
class HydraDataTest(absltest.TestCase):
    ...
    def test_bert_builds_streams(self):
        ...
        try:
            config_name_flag.value = "data/bert/bert"
    ...
    def test_bert_pretraining_loads_data(self):
        # Explicitly select the data YAML config.
        overrides = [
            "data.train.BertNextSentencePredictionInputs.dataset_name=dummy",
            "data.eval.BertNextSentencePredictionInputs.dataset_name=dummy",
            "data.Tokenize.vocab_type=char",
            "data.Tokenize.keys=[0,1]",
            "data.PadToLength.len_map={0:16,1:16,2:16}",
            "data.PadToLength.pad_value={0:0,1:0,2:0}",
            'data.TruncateToLength.len_map={0:"(16,)",1:"(16,)",2:"(16,)"}',
            "data.Batch.batch_size=2",
            "data.Shuffle.queue_size=2",
            "data.mask_random_tokens.masking_prob=1.0",
        ]
        texts = [
            "Alpha beta gamma. Delta epsilon zeta.",
            "Eta theta iota. Kappa lambda mu.",
        ...
        original_dir = config_dir_flag.value
        try:
            config_name_flag.value = "data/bert/bert_pretraining"
            overrides_flag.value = overrides
            config_dir_flag.value = _repo_config_dir()
            cfg = hydra_utils.compose_config()
