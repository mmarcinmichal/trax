...
def build_model():
    ...
    model = tl.Serial(
        tl.Dense(16, use_bias=True), tl.Relu(), tl.Dense(3, use_bias=False)
    )
...
def main():
    ...
    # Load data
    X, y = load_dataset(RawDataset.IRIS.value)
    ...
    test_data, test_labels = load_dataset(
        dataset_name=RawDataset.IRIS.value, split=Splits.TEST.value
    )
