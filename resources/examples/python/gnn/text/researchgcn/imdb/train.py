...
def load_data():
    ...
    x_train = np.stack(
        [_pad_or_trim(t, pad_id=0) for t in tokenizer_fn(iter(train_ds["text"]))]
    )
    y_train = np.array(train_ds["label"], dtype=np.int64)

    x_test = np.stack(
        [_pad_or_trim(t, pad_id=0) for t in tokenizer_fn(iter(test_ds["text"]))]
    )
    y_test = np.array(test_ds["label"], dtype=np.int64)

    # Base window graph (shared for all docs), add self-loops ONCE here:
    base_adj = window_adjacency(
        length=MAX_LEN, window_size=WINDOW_SIZE, add_self_loops=True
    ).astype(np.float32)
