...
def NewLogProbs(dist_inputs, actions, log_prob_fun):
    ...
    new_log_probs = log_prob_fun(dist_inputs, actions)
...
def ApproximateKLDivergence(dist_inputs, actions, old_log_probs, log_prob_fun):
    ...
    approximate_kl_divergence = 0.5 * jnp.mean(new_log_probs - old_log_probs) ** 2
...
def ClippedObjective(probs_ratio, advantages, epsilon):
    ...
    clipped_objective = jnp.clip(probs_ratio, 1 - epsilon, 1 + epsilon) * advantages
