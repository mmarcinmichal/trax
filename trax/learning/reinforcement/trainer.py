...
class RLTrainer:
    ...
    def train_epoch(self):
        """Trains this RLTrainer for one epoch -- main RL logic goes here."""
        raise NotImplementedError
...
class PolicyTrainer(RLTrainer):
    """RLTrainer that uses a deep learning model for policy.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        policy_train_task = supervised_trainer.TrainingTask(
        ...
        policy_eval_task = supervised_trainer.EvaluationTask(
        ...
        self._policy_loop = supervised_trainer.Loop(
...
class LoopPolicyTrainer(RLTrainer):
    """Base class for policy-only trainers based on Loop."""
...
class PolicyGradient(LoopPolicyTrainer):
...
def sharpened_network_policy(temperature, temperature_multiplier=1.0, **kwargs):
    ...
    Args:
      temperature: Temperature passed from the RLTrainer.
...
class ExpertIteration(LoopPolicyTrainer):
...
def network_policy(
    ...
    Used to implement RLTrainer.policy() in policy-based agents.
    ...
    Args:
      ...
      loop: trax.learning.trainer.Loop used to train the policy network
...
class _StepScaledLoop:
    """Proxy around trax.learning.trainer.Loop exposing epoch-aligned step counts."""
